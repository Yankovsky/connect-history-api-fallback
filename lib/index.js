...
exports = module.exports = function historyApiFallback (req, res, next) {
  ...
  for (var pathname in rewriteRules) {
    var target = rewriteRules[pathname];
    if (parsedUrl.pathname == pathname) {
      logger('Rewriting %s %s to %s because of special rewrite rule.',
...
module.exports.setRewriteRules = function (newRewriteRules) {
